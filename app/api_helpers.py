...
async def _call_with_retry_on_429(call_coro_func, max_attempts: int = 15, delay_seconds: float = 12.0):
    """
        ...
        if not is_gemini_response_valid(response_obj_call):
            ...
            if hasattr(response_obj_call, 'candidates'):
                ...
                if response_obj_call.candidates and len(response_obj_call.candidates) > 0:
                    candidate = response_obj_call.candidates[0] if isinstance(response_obj_call.candidates, list) else response_obj_call.candidates
                    if hasattr(candidate, 'content'):
                        ...
                        if hasattr(candidate.content, 'parts'):
                            ...
                            if candidate.content.parts and len(candidate.content.parts) > 0:
                                part = candidate.content.parts[0] if isinstance(candidate.content.parts, list) else candidate.content.parts
